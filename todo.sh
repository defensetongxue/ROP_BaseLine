python train.py --cfg ./configs/defalut.json --split_name 1
# python train.py --cfg ./configs/defalut.json --split_name 2
python train.py --cfg ./configs/defalut.json --split_name 3
python train.py --cfg ./configs/defalut.json --split_name 4

python train.py --cfg ./configs/resnet50.json --split_name 1
python train.py --cfg ./configs/resnet50.json --split_name 2
python train.py --cfg ./configs/resnet50.json --split_name 3
python train.py --cfg ./configs/resnet50.json --split_name 4

python train.py --cfg ./configs/resnet18.json --split_name 1
python train.py --cfg ./configs/resnet18.json --split_name 2
python train.py --cfg ./configs/resnet18.json --split_name 3
python train.py --cfg ./configs/resnet18.json --split_name 4

python train.py --cfg ./configs/resnet18.json --split_name 1
python train.py --cfg ./configs/resnet18.json --split_name 2
python train.py --cfg ./configs/resnet18.json --split_name 3
python train.py --cfg ./configs/resnet18.json --split_name 4

python ring.py
shutdown